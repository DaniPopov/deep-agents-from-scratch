...
from prompts import (
    SYSTEM_PROMPT,
    LAPTOP_AGENT_PROMPT,
    WORKSTATION_AGENT_PROMPT,
    MONITOR_AGENT_PROMPT
)
from state import DeepAgentState
from task_tool import SubAgent
...
# Sub-agents
laptop_agent = SubAgent(
    name="laptop_agent",
    description="Specialized in laptop hardware (gaming laptops, ultrabooks, business laptops, 2-in-1s). Delegate laptop-related research questions here.",
    prompt=LAPTOP_AGENT_PROMPT,
    tools=["search_web", "write_todos", "read_todos", "ls", "read_file", "write_file"]
)

workstation_agent = SubAgent(
    name="workstation_agent",
    description="Specialized in desktop workstations and PC builds (CPUs, GPUs, components, custom builds). Delegate desktop/workstation research here.",
    prompt=WORKSTATION_AGENT_PROMPT,
    tools=["search_web", "write_todos", "read_todos", "ls", "read_file", "write_file"]
)

monitor_agent = SubAgent(
    name="monitor_agent",
    description="Specialized in monitors and docking stations (gaming monitors, professional displays, connectivity). Delegate monitor/docking station research here.",
...
if __name__ == "__main__":
    ...
    # Sub-agents tools
    sub_agent_tools = [search_web, write_todos, read_todos, ls, read_file, write_file]

    # Create task tool to delegate tasks to sub-agents
    task_tool = _create_task_tool(
        sub_agent_tools, [laptop_agent, workstation_agent, monitor_agent], model, DeepAgentState
    )
    ...
    conversation_state = {"messages": [], "todos": [], "files": {}}
    ...
    while True:
        ...
        # Display files if any were added in this turn
        if result.get("files"):
            console.print("\n[bold yellow]🔍 Files Saved to State:[/bold yellow]")
